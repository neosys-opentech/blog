...
+ Production-first : votre produit doit atterrir en production le plus tôt possible avec le minimum viable, pas question qu'il y soit pour la 1ère fois qu'à la fin du projet. Pas mal d'issues bloquante ne sont constatées que lorsqu'on est en production, exemple: les défauts d'architecture ou la non utilisabilité du produit ne peuvent pas se manifester sur les environnements de dév et test. (there's no place like production : Dorothy wizard of Oz)
